...
import com.example.aaa.product.controller.ProductController;
...
public class OrderController {
	...
	@GetMapping("/confirmAndPay/{id}")
	public String confirmAndPayment(@AuthenticationPrincipal UsersDetails userD,Model model, @PathVariable("id") Integer id) {
	...
	@PostMapping("/confirmAndPay/{id}")
	public String proccessPayment(@AuthenticationPrincipal UsersDetails userD,@PathVariable("id") Integer id, ShoppingCart cc) {
	...
	private void sendOrderToAdmin(String destinationEmail2, Order order1) throws UnsupportedEncodingException, MessagingException {
		...
		String content =  "New order with Id :" + order1.getId() + " was completed ..."
				...
				+ "<p> ordered these products </p>" 
				//+ "<p><b>"+order1.getProducts() + "</b></p>"
				+ "<p><b>"+order.getPid() + ", "+order.getName()+", "+order.getQty() + "</b></p>"
				+"<p> that needs to be shipped </p>"
				+"<p>"+ order1.getShipping() + "</p>"
				+"<h1>jhdhfkjshdjfkdsj</h1>"
				;
